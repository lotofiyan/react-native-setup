...
export function GSButton({
  ...
  return (
    <Pressable accessibilityRole="button" {...props} className={buttonClasses}>
      ...
      <Text className={`text-base font-semibold ${textColor} ${textClassName}`}>
        {label}
      </Text>
    </Pressable>
  );
...
type ChipProps = Classed<PressableProps> & {
  label: string;
  icon?: keyof typeof Ionicons.glyphMap;
  id?: string;
  testID?: string;
...
export function GSChip({
  ...
  const resolvedTestID = testID ?? id;
  return (
    <Pressable
  ...
  return (
    <View className="rounded-2xl border border-outline-100 bg-background-50 px-4 py-3">
      ...
      <View className="mt-1 flex-row items-end justify-between">
        ...
        {delta ? (
          <Text
            className={`rounded-full px-2 py-1 text-xs font-semibold ${accent}`}
          >
            {delta}
          </Text>
        ) : null}
